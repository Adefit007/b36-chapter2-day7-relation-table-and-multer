...
app.get("/", function (req, res) {
  ...
  db.connect(function (err, client, done) {
    ...
    if (req.session.isLogin == true) {
      query = `SELECT tb_projects.id, tb_user.name as author,tb_projects.name, start_date, end_date, description, technologies, image, post_at
      FROM tb_projects 
      LEFT JOIN tb_user 
...
app.get("/project-detail/:id", function (req, res) {
  ...
  db.connect(function (err, client, done) {
    ...
    client.query(
      `SELECT * FROM tb_projects WHERE id=${id};`, //medapatkan data pada id tertentu dalam db
      function (err, result) {
        if (err) throw err;
...
app.get("/delete-project/:id", function (req, res) {
  ...
  db.connect(function (err, client, done) {
    if (err) throw err;
    const query = `DELETE FROM tb_projects WHERE id=${id};`; //query menghapus data berdasarkan parameter id
    client.query(query, function (err, result) {
      if (err) throw err;
      res.redirect("/");
    });
...
// POST //
app.post("/project", upload.single("uploadImage"), function (req, res) {
  if (!req.session.user) {
    req.flash("danger", "Login dulu woy...!!!");
    return res.redirect("/login");
  }
  ...
  db.connect(function (err, client, done) {
    ...
    const query = `INSERT INTO tb_projects(name, start_date, end_date, description, technologies, image, author_id)
    VALUES ( '${data.inputProjectName}', '${data.inputStartDate}', '${data.inputEndDate}', '${data.inputDescription}',ARRAY['${technologies[0]}','${technologies[1]}','${technologies[2]}','${technologies[3]}'], '${image}',${authorId});`;
...
app.post(
  ...
  function (req, res) {
    ...
    db.connect(function (err, client, done) {
      ...
      const query = `UPDATE tb_projects SET name= '${data.inputProjectName}', start_date='${data.inputStartDate}', end_date='${data.inputEndDate}', description='${data.inputDescription}', technologies=ARRAY['${technologies[0]}','${technologies[1]}','${technologies[2]}','${technologies[3]}'], image='${image}',author_id=${authorId}
    ...
    client.query(query, function (err, result) {
      ...
      if (data.length != 0) {
        ...
        db.connect(function (err, client, done) {
          ...
          client.query(query, function (err, result) {
            if (err) throw err;
            req.flash("regissuccess", "Register success, login please...");
            res.redirect("/login");
          });
        });
      }
...
app.post("/login", function (req, res) {
  ...
  db.connect(function (err, client, done) {
    ...
    client.query(query, function (err, result) {
      ...
      // console.log(isMatch);
      if (isMatch == false) {
        req.flash("dangerpass", "Upss...!!??, Password not Match..!");
        return res.redirect("/login");
      }
